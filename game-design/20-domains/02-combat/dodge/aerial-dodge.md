...
Khi người chơi đã mở Aerial Dodge, còn aerial-dodge use trong `AerialChain`, đủ Vigor và bấm Dodge ở airborne context hợp lệ, actor né có hướng với defensive window riêng—có thể nối một lần với Aerial Step nhưng không chạy khi bám tường, reset khi wall-touch hoặc trở thành miễn nhiễm rơi/hazard.
...
- In scope: fresh Dodge intent, 360° horizontal/limited vertical presentation, Vigor, defensive categories, collision, Step combo, Grounding/Lightness/Climb conflicts, authority/cue/test.
- Non-goals: wall dodge khi attached, free fall immunity, reset fall damage, repeated air dash, zero-Vigor progression, attack hitbox, quyết định unlock trước `DB-057`.

## 3. Eligibility và lifecycle

```text
AIRBORNE + fresh Dodge + use/Vigor/control/clearance
  → AIR_DODGE_START
  → AIR_DODGE_ACTIVE
  → AIR_DODGE_RECOVERY
...
- Commit trả Vigor đúng một lần theo `DB-057`. Nó không dùng Focus, không đặt grounded, không xóa fall attribution và không tự cho lift ngoài collision-safe trajectory profile.
...
|---|---|
| Aerial Step → Dodge | accept sau step impulse marker nếu use/Vigor còn; Dodge tiếp quản movement/defense |
...
- Start/active/end và Vigor insufficient/collision có visual + audio/body cue; gravity shaping không cần HUD riêng. Reduced motion bỏ trail/shake nhưng giữ direction/window đọc được.

## 7. Scenario seed

| ID | Scenario | Oracle |
|---|---|---|
| `ADO-S-001` | jump/fall/wall-eject → Dodge neutral/4 hướng/diagonal | one action/Vigor cost, basis snapshot, collision-safe |
...
Risk `PR-2`; automated proof cần direction/collision/gravity-reentry/fall-attribution/one-use/resource/attack-category/chain/latency/lifecycle fixtures trước one-question human feel card. Exact displacement, gravity curve, window, Vigor cost, cancel markers, perfect reward và unlock đều `OPEN`; Feature giữ `DISCOVERY`, không cho phép implementation.

Cross-discipline closure bắt buộc:

| Discipline | Direction |
|---|---|
| Input/UI/localization | remapped Dodge + direction, Vigor cost/spent/denied cue và subtitle keys; không hard-code phím hoặc chỉ dùng màu |
