...
## 3. Nội dung từ corpus cũ đã nhập đúng trạng thái

| Candidate | Nguồn archive | Nhãn mới |
|---|---|---|
| action remappable, không hardcode phím | combat 05.2 | `DECIDED` ở mức nguyên tắc, contract input chưa duyệt |
| tám hướng local, neutral backstep | combat 05.2 | `CANDIDATE` |
| dodge ưu tiên hơn recovery chung | combat 05.2 | `CANDIDATE`, cần cancel matrix |
| chỉ thay horizontal velocity khi ở trên không | combat 05.2 | `CANDIDATE` |
| Focus cost 25, tổng chuyển động 0.35s, i-frame ≤0.40s | formula 14.18–19 | `CANDIDATE PARAMETER`, chưa có evidence |
| perfect dodge tick 0–1, hoàn Fatigue/hồi Mana | formula 14.19 | `CANDIDATE`, phụ thuộc resource philosophy |
| client intent/server authority | architecture/test | `DECIDED` ở mức platform principle; prediction còn mở |
...
Corpus cũ dùng Focus cho nhịp action và Fatigue cho phiên săn, đồng thời perfect dodge hoàn Fatigue/hồi Mana. Cần trả lời:
...
`FEAT-COMBAT-DODGE` ở `DISCOVERY`. M1-05 trong backlog cũ không được dùng để bắt đầu code. Bước hợp lệ tiếp theo là research/prototype plan trong `research-plan.md` và hoàn thành scenario matrix.
