...
- **Vigor/Khí Lực:** meter traversal fixed-width chỉ hiện khi leo/Khinh Công, vừa tiêu hoặc recovery liên quan; khác icon/pattern với Focus. Semantic canonical tại [`CTR-TRAVERSAL`](09-traversal-state-surface-and-vigor-contract.md).
...
- Vigor dùng cùng max-change/no-free-refill principle; exact persistence/recovery và traversal state cleanup do `CTR-TRAVERSAL` sở hữu.
