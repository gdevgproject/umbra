...
| `CTR-DEBUG-OBSERVABILITY` | QA/Tools | all systems | causal envelope/reason, event trace, state inspector, bounded replay seed, diagnostic/privacy/recovery và perf correlation | `DISCOVERY` |
...
| `CTR-SHADOW-RETINUE` | Shadows + Systems | capture, identity, deployment, AI, UI, balance, save | slot 1→8, active cap 4, full-roster claim, membership lifecycle | `PROPOSED` |
| `CTR-VITALS-HUD` | Combat Systems + Player Traversal + UI/UX | damage, Dodge, skill, item recovery, world hazard, death | one authoritative HP, one Vigor action resource, fixed-width HUD, Minecraft hunger/environment mapping | `PROPOSED` |
| `CTR-TRAVERSAL` | Player Traversal + Gameplay Tech | Free Climb, Grounding Strike, Lightness, world, camera, animation, save | locomotion states, Vigor, surface/ledge, fall transition, authority | `PROPOSED` |
...
- [HP, Vigor và HUD](08-vitals-resource-and-hud-contract.md)
- [Trạng thái traversal, bề mặt và Khí Lực](09-traversal-state-surface-and-vigor-contract.md)
