...
## 3. Performance envelope thay cho “mọi entity full AI”

Stress fantasy hiện tại là player quan sát khoảng 40 Shadow và 80 hostile. Đây là **số actor biểu đạt trong trận**, không phải lời hứa 120 actor cùng chạy perception, pathfinding, animation, audio và VFX ở fidelity cao nhất.
...
- Shadow command ưu tiên squad intent rồi mới fan-out; command không broadcast toàn roster nếu chỉ một nhóm liên quan.
...
| `PERF-S02` | arena 40 Shadow + 80 hostile theo đủ tier | combat, query, command, VFX, path |
...
- `DB-018`: composition/tier/cue cụ thể của stress scene 40/80.
