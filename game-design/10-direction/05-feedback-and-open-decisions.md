...
| `PD-034` | Mana và Focus là hai resource riêng; Focus hồi nhanh cho phòng thủ chủ động, không là stamina di chuyển chung | HUD fixed-width có cost preview/notch; exact cap/regen/cost vẫn do `DB-046` và `DB-042` chứng minh |
...
| `PD-039` | Traversal core là năng lực phổ quát mở rất sớm bằng milestone + training, không là Potential/build choice | basic mantle candidate có từ đầu; exact level/quest do `DB-049`, `DB-050`, `DB-051`, `DB-052`; implementation snapshot dùng capability grant trước Quest Kernel |
...
| `PD-047` | Future Double Jump phải là Nhịp Không Trung có thể chain thật với Aerial Dodge nhưng không nhập hai action hoặc hai resource | tách `FEAT-TRAVERSAL-AERIAL-STEP` và `FEAT-COMBAT-AERIAL-DODGE`; một `AerialChain` cho tối đa bounded use mỗi kỹ thuật, Vigor/Focus riêng, stable support mới reset; wall touch/Climb không refill và attached/mantling không được coi là airborne/Grounding-eligible |
