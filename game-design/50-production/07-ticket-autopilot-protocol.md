...
| User đưa feedback mới khi ticket đang chạy | capture `CR`, atomize observation/outcome, triage theo `GOV-ROUTING`; không coi là pass, không tự scope-creep |
...
### Feedback interrupt không làm mất ticket context

Feedback mới là một interrupt có kiểm soát, không mặc định thay ticket hoặc tạo ticket mới:

1. giữ active ticket/branch/dirty evidence; ghi `CR-<TKT>-NN` và pointer trong ledger;
2. so `OBSERVED/EXPECTED/REQUESTED` với `design_baseline_commit` và exact canonical owners;
3. chọn disposition từ `GOV-ROUTING`;
4. conformance fix tiếp tục cùng ticket; local amend rebaseline docs→ticket→tests→code; follow-up không scope-creep; shared/irreversible delta chuyển `NEEDS_DESIGN`; incident rollback/disable;
5. chỉ trở lại `IN_PROGRESS/AUTOMATED_VERIFYING` sau khi acceptance/risk/evidence baseline mới nhất quán.

User có thể thích một prototype nhưng verdict đó chỉ là feel evidence. Nó không tự approve contract, invalidate automated failure hoặc biến spike/workaround thành production implementation.
...
- mọi `CR`, disposition, canonical delta, stale/rerun evidence và downstream ticket/capability impact.
