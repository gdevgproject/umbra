...
| `C1-FOUNDATION-KERNEL` | mod shell 26.2 mở integrated/dedicated test world, save/reload/quit/backup–restore sạch | pinned baseline/adapters, logical-server authority, stable IDs, content/config validation, schema/migration/portable backup, feature flags, Việt/Anh resource skeleton, diagnostics, test source sets, Git workflow | lifecycle/restore smoke, migration fixture, diagnostics export, CI/test/compat manifest |
| `C2-PLAYER-EMBODIMENT` | đi/chạy/nhìn/mine/place/interact ở góc một và camera UMBRA; sau gate profile mới default góc ba | versioned action catalog/key baseline, ergonomic input context, camera rig/occlusion/aim ray, pose/animation seam, UI tokens/localization/accessibility, creative/survival policy | key/conflict matrix, camera room screenshots, locale/mode/lifecycle tests, user comfort card |
...
| `C8-RETURNING-WORLD` | activity thay đổi home/hub/quest/economy/NPC response | rich quest verticals, world mutation, production rewards/source-sink, family/narrative, localization/assets | continuity/reload/source-sink/world consequence/emotional proof |
